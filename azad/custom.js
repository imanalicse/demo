$(document).ready(function(){
    ...
    function placeOrder(){
        $(".place-order").on('click', function(e) {
            var wa_cart = getSessionData('wa_cart');
            console.log(wa_cart);
            $('#cart').val(JSON.stringify(wa_cart));
            $('#processOrder').submit();
        });
    }
    
    function continueShoping() {
        $(".continue-shoping").on('click', function(e) {
            $(location).attr('href','https://records-primo-sb.hosted.exlibrisgroup.com/primo-explore/search?query=any,contains,school&tab=default_tab&search_scope=default_scope&sortby=rank&vid=61SRA&lang=en_US');
        });
    }
                ...
                if(response){
                    ...
                    var popupHtml = '<div class="popup-accordion" id="accordion">';
                        ...
                        popupHtml +='<div class="popup-footer text-center">';
                            popupHtml += '<form action="http://localhost/nsw/demo/action.php" method="post" name="processOrder" id="processOrder">';
                            popupHtml += '<input type="text" name="cart" id="cart">';
                            popupHtml +='<button type="button" class="btn btn-continue continue-shoping">Continue Shopping</button>' +
                                '<button type="button" class="btn btn-brand place-order">Place order</button>';
                            popupHtml += '</form>';
                        popupHtml +='</div>';
